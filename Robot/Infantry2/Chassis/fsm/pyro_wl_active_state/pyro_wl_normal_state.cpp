...
{
    owner->_ctx.data.flag.aerial_cnt = 0;
    owner->_ctx.data.flag.is_aerial = 0;
    owner->_ctx.data.flag.test = 0;
    speed_offset_ramp = 0.0f;
    spin_decay_speed = owner->_ctx.data.g_yaw; /* Initialize spin decay velocity with current yaw rate / 记录进入时的偏航转速 */
}
...
void wl_chassis_t::fsm_active_t::state_normal_t::execute(wl_chassis_t *owner)
{
    ...
    clear_cnt++;
    if((clear_cnt > 5000)&&(owner->_ctx.cmd->vx == 0.0f))
    {
        clear_cnt = 0;
    }
    ...
    if(!owner->_ctx.data.flag.is_aerial)
    {
        /* On ground check takeoff / 地面状态，判定是否飞起 */
        if(owner->_ctx.data.leg[wl_chassis_t::R].P < TAKEOFF_FORCE_THRESHOLD
            || owner->_ctx.data.leg[wl_chassis_t::L].P < TAKEOFF_FORCE_THRESHOLD)
        {
            owner->_ctx.data.flag.aerial_cnt++;
            if(owner->_ctx.data.flag.aerial_cnt >= AERIAL_DEBOUNCE)
            {
                owner->_ctx.data.flag.is_aerial = 1;
                owner->_ctx.data.flag.aerial_cnt = 0;
            }
        }
        else
    ...
    else
    {
        ...
        const bool leg_compressed =
            owner->_ctx.data.leg[wl_chassis_t::R].l
                < (owner->_ctx.data.leg[wl_chassis_t::R].ref_l - LANDING_COMPRESSION_THRESHOLD)
            || owner->_ctx.data.leg[wl_chassis_t::L].l
                < (owner->_ctx.data.leg[wl_chassis_t::L].ref_l - LANDING_COMPRESSION_THRESHOLD);
        const bool upward_impact =
            owner->_ctx.data.accel_upward_lpf > LANDING_UPWARD_ACC_THRESHOLD;

        if(leg_compressed && upward_impact)
        {
            owner->_ctx.data.flag.aerial_cnt++;
            if(owner->_ctx.data.flag.aerial_cnt >= LANDING_DEBOUNCE)
            {
                owner->_ctx.data.flag.is_aerial = 0;
                owner->_ctx.data.flag.aerial_cnt = 0;
                owner->_ctx.data.flag.test = 1;
                /* Lock current length as reference immediately upon landing
                   落地瞬间，立刻获取当前实际伸缩长作为闭环基准高度，防止反弹摔倒 */
                owner->get_cur_length(&owner->_ctx.data.leg[wl_chassis_t::R].ref_l, &owner->_ctx.data.leg[wl_chassis_t::L].ref_l);
            }
        }
        else
        {
    ...
    if (fabsf(spin_decay_speed) > decay_step) {
        ...
        g_yaw_ref = owner->_ctx.pid.yaw->calculate(0.0f, -owner->_ctx.data.gimbal_yaw);
    }
    ...
    if (fabsf(owner->_ctx.data.gimbal_yaw) < 0.05f) {
       owner->_ctx.data.leg[wl_chassis_t::R].T_w_turn  = wheel_turn_pid_soft[wl_chassis_t::R].calculate(g_yaw_ref, -owner->_ctx.data.gimbal_g_yaw);
       owner->_ctx.data.leg[wl_chassis_t::L].T_w_turn  = wheel_turn_pid_soft[wl_chassis_t::L].calculate(g_yaw_ref, -owner->_ctx.data.gimbal_g_yaw);
    } else {
       owner->_ctx.data.leg[wl_chassis_t::R].T_w_turn = turn_pid[wl_chassis_t::R].calculate(g_yaw_ref, owner->_ctx.data.g_yaw);
       owner->_ctx.data.leg[wl_chassis_t::L].T_w_turn = turn_pid[wl_chassis_t::L].calculate(g_yaw_ref, owner->_ctx.data.g_yaw);
    }
    ...
    owner->_ctx.data.roll_gain = owner->_ctx.pid.roll->calculate(0.0f, owner->_ctx.data.roll);
    ...
    for(uint8_t i = 0; i < 2; i++)
    {
        ...
        if(owner->_ctx.data.flag.is_aerial)
        {
            ...
            owner->_ctx.data.leg[i].T_w_balance = 0.0f;
            owner->_ctx.data.leg[i].T_w_move = 0.0f;
            owner->_ctx.data.leg[i].T_w_turn = 0.0f;

            /* Set length reference dynamically to stretch / 腾空拉伸腿长 */
            if(0.005f < abs(0.37f - owner->_ctx.data.leg[wl_chassis_t::R].ref_l))
            {
                owner->_ctx.data.leg[wl_chassis_t::R].ref_l += 0.001f;
            }
            else
            {
                owner->_ctx.data.leg[wl_chassis_t::R].ref_l = 0.32f;
            }
            if(0.005f < abs(0.37f - owner->_ctx.data.leg[wl_chassis_t::L].ref_l))
            {
                owner->_ctx.data.leg[wl_chassis_t::L].ref_l += 0.001f;
            }
            else
            {
                owner->_ctx.data.leg[wl_chassis_t::L].ref_l = 0.32f;
            }

            /* Contraction/extension control under fly state / 腾空极径闭环计算 */
            owner->_ctx.data.leg[wl_chassis_t::R].ref_d_l =
                aerial_pid[wl_chassis_t::R].calculate(owner->_ctx.data.leg[wl_chassis_t::R].ref_l, owner->_ctx.data.leg[wl_chassis_t::R].l);
            owner->_ctx.data.leg[wl_chassis_t::R].F[0] =
                aerial_d_pid[wl_chassis_t::R].calculate(owner->_ctx.data.leg[wl_chassis_t::R].ref_d_l, owner->_ctx.data.leg[wl_chassis_t::R].d_l)-30.0f;

            owner->_ctx.data.leg[wl_chassis_t::L].ref_d_l =
                aerial_pid[wl_chassis_t::L].calculate(owner->_ctx.data.leg[wl_chassis_t::L].ref_l, owner->_ctx.data.leg[wl_chassis_t::L].l);
            owner->_ctx.data.leg[wl_chassis_t::L].F[0] =
                aerial_d_pid[wl_chassis_t::L].calculate(owner->_ctx.data.leg[wl_chassis_t::L].ref_d_l, owner->_ctx.data.leg[wl_chassis_t::L].d_l)-30.0f;
        }
        else
        {
            ...
            else
            {
                ...
                if(0.005f < abs(test_length - owner->_ctx.data.leg[wl_chassis_t::R].ref_l))
                {
                    if(test_length < owner->_ctx.data.leg[wl_chassis_t::R].ref_l)
                    {
                        owner->_ctx.data.leg[wl_chassis_t::R].ref_l -= 0.001f;
                    }
                    else
                ...
                {
                    owner->_ctx.data.leg[wl_chassis_t::R].ref_l = test_length;
                }
                if(0.005f < abs(test_length - owner->_ctx.data.leg[wl_chassis_t::L].ref_l))
                {
                    if(test_length < owner->_ctx.data.leg[wl_chassis_t::L].ref_l)
                    {
                        owner->_ctx.data.leg[wl_chassis_t::L].ref_l -= 0.001f;
                    }
                    else
                ...
                {
                    owner->_ctx.data.leg[wl_chassis_t::L].ref_l = test_length;
                }
                if((owner->_ctx.data.leg[wl_chassis_t::R].ref_l == test_length) && (owner->_ctx.data.leg[wl_chassis_t::L].ref_l == test_length))
                {
                    owner->_ctx.data.flag.test = 0;
                }
            }
            ...
            owner->_ctx.data.leg[i].d_x_bias = (0.5f * speed_offset_ramp) + owner->_ctx.data.leg[i].d_x_gain - owner->_ctx.data.leg[i].kf_v;
            owner->_ctx.data.leg[i].beta_bias = 0.05f - owner->_ctx.data.leg[i].beta;
            owner->_ctx.data.leg[i].d_beta_bias = 0.0f - owner->_ctx.data.leg[i].d_beta;
            ...
            /* Calculate wheel motion torque from LQR (x + dx states)
               轮毂行驶反馈：由位移和速度状态误差确定 */
            owner->_ctx.data.leg[i].T_w_move = (
    ...
    owner->_ctx.data.leg[wl_chassis_t::R].F[1] = fp32_constrain(owner->_ctx.data.leg[wl_chassis_t::R].F[1], -200.0f, 200.0f);
    owner->_ctx.data.leg[wl_chassis_t::L].F[1] = fp32_constrain(owner->_ctx.data.leg[wl_chassis_t::L].F[1], -200.0f, 200.0f);
...
void wl_chassis_t::fsm_active_t::state_normal_t::calc_support_force(wl_chassis_t *owner)
{
    for(uint8_t i = 0; i < 2; i++)
    {
        owner->_ctx.data.leg[i].P
         = owner->_ctx.data.leg[i].F[0] * arm_cos_f32(owner->_ctx.data.leg[i].beta)
         + owner->_ctx.data.leg[i].F[1] * arm_sin_f32(owner->_ctx.data.leg[i].beta) / owner->_ctx.data.leg[i].l
         + owner->_ctx.data.accel_upward_lpf
         - owner->_ctx.data.leg[i].d2_l * arm_cos_f32(owner->_ctx.data.leg[i].beta)
         + 2.0f * owner->_ctx.data.leg[i].d_l * owner->_ctx.data.leg[i].d_beta * arm_sin_f32(owner->_ctx.data.leg[i].beta)
         + owner->_ctx.data.leg[i].l * owner->_ctx.data.leg[i].d2_beta * arm_sin_f32(owner->_ctx.data.leg[i].beta)
         + owner->_ctx.data.leg[i].l * owner->_ctx.data.leg[i].d_beta * owner->_ctx.data.leg[i].d_beta * arm_cos_f32(owner->_ctx.data.leg[i].beta);
    }
}
}
