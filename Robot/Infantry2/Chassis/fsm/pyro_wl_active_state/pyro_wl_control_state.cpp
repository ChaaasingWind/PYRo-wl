...
void wl_chassis_t::fsm_active_t::state_control_t::execute(wl_chassis_t *owner)
{
    ...
    {
            owner->_ctx.data.leg[i].T_w_move = wheel_disable_pid[i].calculate(0.0f,
                 owner->_ctx.data.leg[i].w);
    ...
    /* Right leg / 右腿 */
    float diff;
    if(owner->_ctx.cmd->r_angle - owner->_ctx.data.leg[wl_chassis_t::R].alpha > PI)
    {
        diff = -2 * PI + (owner->_ctx.cmd->r_angle - owner->_ctx.data.leg[wl_chassis_t::R].alpha);
    }
    else if(owner->_ctx.cmd->r_angle - owner->_ctx.data.leg[wl_chassis_t::R].alpha < -PI)
    {
        diff = 2 * PI + (owner->_ctx.cmd->r_angle - owner->_ctx.data.leg[wl_chassis_t::R].alpha);
    }
    else
    {
        diff = owner->_ctx.cmd->r_angle - owner->_ctx.data.leg[wl_chassis_t::R].alpha;
    }
    ...
    /* Left leg / 左腿 */
    if(owner->_ctx.cmd->l_angle - owner->_ctx.data.leg[wl_chassis_t::L].alpha > PI)
    {
        diff = -2 * PI + (owner->_ctx.cmd->l_angle - owner->_ctx.data.leg[wl_chassis_t::L].alpha);
    }
    else if(owner->_ctx.cmd->l_angle - owner->_ctx.data.leg[wl_chassis_t::L].alpha < -PI)
    {
        diff = 2 * PI + (owner->_ctx.cmd->l_angle - owner->_ctx.data.leg[wl_chassis_t::L].alpha);
    }
    else
    {
        diff = owner->_ctx.cmd->l_angle - owner->_ctx.data.leg[wl_chassis_t::L].alpha;
    }
