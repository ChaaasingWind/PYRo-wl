...
void wl_chassis_t::fsm_active_t::on_enter(wl_chassis_t *owner)
{
    ...
    {
        if(dm_motor_drv_t::ok != owner->_ctx.motor.joint[i]->get_error_code())
        {
            owner->_ctx.motor.joint[i]->clear_error();
        }
        owner->_ctx.motor.joint[i]->enable();
    }

    /* Enable 2 wheel motors / 使能左右轮毂电机 */
    for(uint8_t i = 0; i < 2; i++)
    {
        owner->_ctx.motor.wheel[i]->enable();
    }
