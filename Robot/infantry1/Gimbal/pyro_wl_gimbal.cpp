...
void wl_gimbal_t::_update_feedback()
{
    ...
    _ctx.data.telem.targetYawRotate    = _current_cmd.yawVel;
    _ctx.data.telem.targetPitchRotate  = _current_cmd.pitchVel;
...
void wl_gimbal_t::updatePitch()
{
    ...
    //再还原回imu的角度用来pid计算
    targetMotorRaw = -(targetMotorRaw - offsetPitch);

    //云台俯仰轴的重力补偿和位置控制
    float gravityFf           = PITCH_K_GRAVITY_COS * arm_cos_f32(_ctx.data.imu.pitch) + PITCH_K_GRAVITY_SIN * arm_sin_f32(_ctx.data.imu.pitch);

    float target_pitch_speed   = _module_deps.pid_deps.pitch_pos->calculate(targetMotorRaw, _ctx.data.imu.pitch);
    //float target_pitch_pid_out = _module_deps.pid_deps.pitch_spd->calculate(target_pitch_speed, _ctx.data.imu.gyro[1]);


    _ctx.data.output.pitchTorque = target_pitch_speed - gravityFf;
...
void wl_gimbal_t::updateYaw()
{
    ...
    //角度归一化
    float TgtYaw   = _ctx.data.imu.yaw + wrapAngle(_ctx.data.telem.targetYawRad - _ctx.data.imu.yaw);
    float yawPosOut       = _module_deps.pid_deps.yaw_pos->calculate(TgtYaw, _ctx.data.imu.yaw);
...
{
    _module_deps.motor_deps.pitch->send_torque(0);
