...
void pyro::wl_booster_t::fsm_active_t::state_spin_up_t::execute(owner* owner)
{
    ...
    // --- 启动完成检测: 双摩擦轮速度均接近目标 ---
    if (std::abs(owner->_ctx.data.motor_state.fric[0].vel - owner->_ctx.data.target_state.targetFricSpeed) < 10 &&
        std::abs(owner->_ctx.data.motor_state.fric[1].vel - owner->_ctx.data.target_state.targetFricSpeed) < 10) 
    {
